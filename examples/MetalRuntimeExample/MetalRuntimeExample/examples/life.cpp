...
static void genValues(BufferType *buffer, int32_t columns) {
  buffer[0 * columns + 0] = buffer[0 * columns + 1] = buffer[0 * columns + 2] =
...
void lifeMetal() {
  ...
  intptr_t buffer0 =
      _MetalDeviceMakeBuffer(device, isStorageModeManaged, count, typeSize);
  intptr_t buffer1 =
      _MetalDeviceMakeBuffer(device, isStorageModeManaged, 1, typeSize);
  intptr_t buffer2 =
      _MetalDeviceMakeBuffer(device, isStorageModeManaged, count, typeSize);
  ...
  for (int32_t i = 0; i < ITER; ++i) {
    intptr_t commandBuffer = _MetalCommandQueueMakeCommandBuffer(
        commandQueue, libPath, (const int8_t *)"life", rows, columns, 1);
    auto even = i % 2 == 0;
    ...
    if (print) {
      auto result =
          (BufferType *)_MetalBufferGetContents2(even ? buffer2 : buffer0);
